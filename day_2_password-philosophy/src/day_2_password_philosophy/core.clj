...
; --- tests ---
(read-input-coll '("1-2 a: ddfadfa"))
(read-input-coll input)

(defn as-char [s] (get s 0))
...
(defn valid-password?
  ...
  (let
    [low (Integer/parseInt (first datum))
     ...
     letter-freq (if (nil? (get freq letter)) 0 (get freq letter))
     ]
    ;(println datum)
    ;(println "<= " low letter-freq high)
    (<= low letter-freq high)))


(defn valid-passwords
  [data]
  (filter valid-password? data))


; ----- DATA -----
(def data (read-input-coll input))

(println (count (valid-passwords data)))
