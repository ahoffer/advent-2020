(ns day-3-toboggan-trajectory.core)
...
(def test1 (clojure.string/split-lines "..##.......
#...#...#..
.#....#..#.
..#.#...#.#
.#...##..#.
...
(def test2 (clojure.string/split-lines "###
###
###"))
...
(defn grid-value
  ...
  (let [
        ...
        x (mod x num-cols)
        line (get data y)
        character (get line x)
        value (if (= character \.) :open :tree)
        symbol (if (= value :open) "O" "X")]
    (println line "\t" (replace-char line x symbol))
    value))

(defn count-trees
  [x y step-right step-down tree-count data]
  (let
    [next-x (+ x step-right)
     next-y (+ y step-down)
     obj (grid-value x y data)
     tree-count (if (= obj :tree) (inc tree-count) tree-count)
     num-rows (count data)]
    (if (< next-y num-rows)
      (count-trees next-x next-y step-right step-down tree-count data)
      tree-count)))

(count-trees 0 0 3 1 0 input)
(println (= (count-trees 0 0 3 1 0 input) 278))
...
(reduce * (map #(count-trees 0 0 (first %) (second %) 0 input) '((1 1) (3 1) (5 1) (7 1) (1 2))))
;Answer 9709761600
