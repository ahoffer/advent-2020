(ns day-7-handy-haversacks.first-attempt)
...
; (expand-id :shiny-gold sample-rules)
...
(defn contains-particular-bag
  ...
  (for [[bag-id contents] unwound-rules
        :when (contains? contents particular-bag-id)]
    bag-id))

(println "Part 1 Solution" (count (contains-particular-bag :shiny-gold unwound-real-rules)))



(def part-2-sample-rules (map clojure.string/trim
...
; (unwind (make-rules part-2-sample-rules))




; (apply + (vals (:shiny-gold (unwind sample-rules))))
(println "Part 2 Solution" (apply + (vals (unwound-real-rules :shiny-gold))))